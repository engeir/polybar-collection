...
if [[ $my_laptop_external_monitor = *" connected"* ]]; then
    ...
    sleep 0.5
    hideIt.sh -w -i 0.2 -d "top" --name "polybar-top_eDP-1" --peek 0 --region 0x360+1920+3 & disown
    sleep 0.5
    hideIt.sh -w -i 0.2 -d "top" --name "polybar-usbc_DP-2" --peek 0 --region 1920x0+2560+3 & disown
else
    hideIt.sh -w -i 0.2 -d "top" --name "polybar-top_eDP-1" --peek 0 --region 0x0+1920+22 & disown
fi
